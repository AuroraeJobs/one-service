...
final class LotteryBacktestSummarySupport {
    ...
    private static LotteryBacktestSummary summary(LotteryBacktestReport report) {
        ...
        return LotteryBacktestSummary.builder()
                ...
                .decisionSetId(report.getDecisionSetId())
                .provenance(report.getProvenance())
                ...
                .baselineSeed(report.getBaselineSeed())
                .baselineAlgorithm(report.getBaselineAlgorithm())
                .windowIssueCount(report.getWindowIssueCount())
                .candidateCount(report.getCandidateCount())
                .uniqueCandidateCount(report.getUniqueCandidateCount())
                .ticketCount(report.getTicketCount())
                .baselineTicketCount(report.getBaselineTicketCount())
                .sameWindow(report.getSameWindow())
                .sameBudget(report.getSameBudget())
                ...
                .roiPercent(report.getRoiPercent() == null ? roiPercent(report.getNetResult(), report.getTotalCost()) : report.getRoiPercent())
                .baselineTotalCost(report.getBaselineTotalCost())
                .baselineTotalPrize(report.getBaselineTotalPrize())
                .baselineNetResult(report.getBaselineNetResult())
                .baselineRoiPercent(report.getBaselineRoiPercent())
                .averageRedHitsDelta(report.getAverageRedHitsDelta())
                .blueHitRateDelta(report.getBlueHitRateDelta())
                .totalPrizeDelta(report.getTotalPrizeDelta())
                .netResultDelta(report.getNetResultDelta())
                .roiPercentDelta(report.getRoiPercentDelta())
                .candidateDiversity(report.getCandidateDiversity())
                ...
                .hitDistribution(report.getHitDistribution() == null ? new LinkedHashMap<>() : new LinkedHashMap<>(report.getHitDistribution()))
                .baselineHitDistribution(report.getBaselineHitDistribution() == null ? new LinkedHashMap<>() : new LinkedHashMap<>(report.getBaselineHitDistribution()))
                .maxRedOverlap(report.getMaxRedOverlap())
                .distinctBlueCount(report.getDistinctBlueCount())
                .evaluationMode(report.getEvaluationMode())
                .overfitWarnings(report.getOverfitWarnings() == null ? new java.util.ArrayList<>() : new java.util.ArrayList<>(report.getOverfitWarnings()))
