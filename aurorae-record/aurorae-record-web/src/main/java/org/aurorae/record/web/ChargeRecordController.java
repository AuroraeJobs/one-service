...
import org.aurorae.record.dto.ChargeLocationDTO;
import org.aurorae.record.dto.ChargeProviderDTO;
...
public class ChargeRecordController {
    ...
    @Operation(summary = "获取充电地点列表", description = "获取所有可选的充电地点枚举")
    public List<ChargeLocationDTO> getLocations() {
        return Arrays.stream(ChargeLocation.values())
                .map(location -> ChargeLocationDTO.builder()
                        .label(location.getLabel())
                        .value(location.getValue())
                        .provider(location.getProvider().name())
                        .build())
