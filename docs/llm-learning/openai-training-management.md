...
当前进展：`/ai/training` 已经提供只读“OpenAI 训练管理”学习页，并通过 one-service 的 `/ai/training/dashboard` API 获取生命周期、管理对象、训练任务、Eval 决策、模型部署绑定和下一步实现数据。

下一步可以把 `/ai/training/dashboard` 的固定学习蓝图替换为 Mongo 持久化数据，再逐步接入 OpenAI 文件、fine-tuning job、checkpoint 和 eval API。
