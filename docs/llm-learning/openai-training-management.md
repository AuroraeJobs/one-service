...
当前进展：`/ai/training` 已经提供只读“OpenAI 训练管理”学习页，并通过 one-service 的 `/ai/training/dashboard` API 获取生命周期、管理对象、数据集资产、训练任务、训练指标快照、Checkpoint 资产、Eval 决策、模型部署绑定、上线门禁检查和下一步实现数据。
