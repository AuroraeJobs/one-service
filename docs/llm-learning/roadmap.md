...
已完成工具：`playground/mini-gpt` 支持 tiny/small/medium 训练预设、独立 run 保存、loss/验证集/生成样例记录、Loss 诊断、语料诊断、生成诊断、下一步实验建议、本机持久化实验计划队列、计划项应用到训练表单、下一轮变量差异预览、单变量实验守卫、多变量一键收敛、单变量实验名辅助、实验名唯一性检查、训练启动检查清单、启动检查入报告、复盘草稿生成、复盘草稿来源、复盘质量检查、复盘质量入报告、实验模板、实验对比、实验对比摘要、复盘问题、复盘笔记、实验报告复制、高级训练参数、参数速查、tensor shape 速查、代码定位与学习动作、checkpoint 生成试验台，以及可选择历史实验的 Web 观察台。`/ai/minigpt` 页面会按 Tokenizer、Batch、Causal Attention、Loss、Generate、Review 给出学习阶段清单。
...
- 在实验笔记里先查看复盘草稿来源和复盘质量检查，再点击 `生成复盘草稿`，改成自己的判断后保存。
