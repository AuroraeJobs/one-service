...
- [Iteration 48 plan](iterations/iteration-48-plan.md): completed MiniGPT temporal-boundary and post-corpus observation evidence across Waves 48A-48D, including final release and browser handoff.
...
- Iterations 1-48 are complete. No next candidate is currently promoted; the next planning review will select one from the long-term themes.
...
- Final Wave 48D route smoke and release evidence pass 1326/1326 checks across 18 routes with zero failures, including dedicated five-state, exact-chain, month-end/CSV, `UNKNOWN` non-PASS, no-parallel-route, localization, and quality-gate guards.

## Current Delivery

Iterations 1-48 are complete. Iteration 47 remains the historical foundation for the MiniGPT research chain; its final implementation and evidence are recorded in `docs/lottery/iterations/iteration-47-plan.md` and the checked Iteration 47 section in `docs/lottery/iterations/checklist.md`.

Iteration 47 covers strategy samples, versioned manifests, deterministic time-based train/validation splits, full-sample context validation, fixed evaluation, traceable candidate batches, typed downstream provenance, deterministic same-window/same-budget random baselines, explicit historical-replay warnings, draft-only ticket-pack handoff, actual-result/ledger attribution, manual lifecycle review, month-end review, and existing CSV release/export evidence:
...
Completed Wave 48C carries that fixed meaning into `/lottery/month-end` and the existing CSV evidence chain. Month-end uses an independent bounded-100 include-archived snapshot, preserves separate decision/issue/candidate/settled-financial/baseline coverage, shows truncation and safety warnings, and stays outside the score and lifecycle state. `decision-outcomes` uses an export-internal Java classifier mirror, decision-level rather than candidate-level boundary ownership, fixed snapshot metadata that must not be summed across flattened candidate rows, and exact-owned reviewed-baseline gates with no latest-report fallback. No new export domain, route, DTO, collection, or business-data write was added.

Wave 48D closes the release and handoff. Focused `LotteryExportServiceTest` coverage passes 12/12, file-scoped ESLint and the 1090-call i18n audit pass, smoke/release evidence passes 1326/1326 across 18 routes, and the production build passes with only the existing large-chunk advisory. Rendered Chinese/English, light/dark, 1280px/390px QA confirms five-state counts `1/1/1/4/1`, observed denominator 4, fixed 100/103 truncation, independent financial/baseline coverage, `UNKNOWN` non-PASS semantics, active existing export preset, and no document/panel horizontal overflow. A real CSV confirms the same snapshot and exact-owner fields; it creates only the existing export audit record, not a new business-data mutation. All tagged fixtures and temporary runtimes are removed, implementation commit `b0c3e3ee` is pushed, and no next candidate is currently promoted.
...
Iteration 10 completed the platform foundation, and later frontend/usability waves are tracked in `docs/lottery/iterations/checklist.md`. Iterations 1-48 and the verified Iteration 48 delivery handoff are complete. No next candidate is currently promoted; the next planning review will select one.
