...
Final Iteration 47 verification: backend service tests passed 96/96, backend web tests passed 43/43, frontend i18n audit checked 1039 calls, lottery smoke passed 1065/1065 checks across 18 routes, release evidence is fresh, the production build passed, and rendered desktop plus 390x844 QA completed without console warnings or horizontal overflow.
...
Formal plan: [Iteration 48 plan](iteration-48-plan.md). Iteration 48 and Waves 48A-48D are complete. No next candidate is currently promoted; the next planning review will select one.
...
- [x] Align iteration/module docs and quality gates after the complete behavior is verified. README, this checklist, the Iteration 48 and long-term plans, menu/version plan, technical design, selection/training strategy, and quality gates now describe the fixed snapshot, exact-owner, repeated-row, browser, cleanup, and no-new-business-mutation contracts consistently.
- [x] Add smoke/release evidence for all five states, exact-chain ownership, observed-only aggregation, month-end/CSV propagation, `UNKNOWN` non-PASS rendering, and no new business-data mutation or parallel route regression. Final release evidence passes 1326/1326 source checks across 18 routed smoke surfaces with zero failures.
- [x] Run focused tests, i18n audit, lottery smoke/release check, file-scoped ESLint, TypeScript/production build, rendered desktop/mobile QA, staged-scope audit, commit, and push. Final evidence: Maven 12/12, ESLint, i18n 1090, production build, Chinese/English light/dark 1280px/390px browser checks, real CSV `1/1/1/4/1` with observed denominator 4 and 100/103 truncation, active existing research preset, no horizontal overflow, zero tagged QA fixtures after cleanup, and pushed implementation commit `b0c3e3ee`; the release-only handoff scope excludes the two local MiniGPT runtime data files.

Iteration 48 is complete. No next candidate is currently promoted; continue with a planning review before adding another iteration.
