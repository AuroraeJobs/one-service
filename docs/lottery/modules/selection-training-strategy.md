...
Iteration 48 是 Iteration 47 完整发布闭环后的唯一晋升候选。已完成的 Wave 48A 解决“这条结果位于语料时间边界的哪里”，已完成的 Wave 48B 在同一页面对真实语料后观察做有界只读汇总。两者都不重新训练、不重新回测，也不根据命中、奖金、ROI 或人工复核动作改变分类。
...
Iteration 47A-47D 及其 production release、浏览器、暂存范围、提交和推送证据已经完成。Iteration 48 是唯一晋升候选，Wave 48A 前端只读五态时间边界和 Wave 48B 有界样本外观察聚合均已完成；下一步推进 Wave 48C 月末/既有 CSV 证据，再进入发布闭环。Wave 48B 最终校验已通过 file-scoped ESLint、i18n 1088、smoke/release 1134/1134（18 routes）和 production build；隔离夹具覆盖五态 `1/1/1/4/1`、100/103 截断、精确可比/FAIL/归属错/不完整溯源，并通过中英文 1280px 及英文 390px 明暗主题、无横向溢出、后端就绪后无新增控制台错误、GET-only 读取及完整清理。在任何波次都不得把 `POST_CORPUS_OBSERVED` 写成表现 PASS 或未来预测承诺，`UNKNOWN` 永不 PASS。
