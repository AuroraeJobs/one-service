...
const HealthFourthPage: React.FC = () => {
  ...
  useEffect(() => {
    if (allRecords && selectedCard === null) {
      let count = 0;
  ...
  return (
    <div className="themed-route-page health-fourth-page" style={{
      minHeight: '100vh', 
      ...
      <div style={{ flex: 1, padding: '20px', marginBottom: '80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px', marginBottom: '24px' }}>
          <Card
            className="health-fourth-container-card"
            style={{
              borderRadius: '20px', 
          ...
          >
            <div style={{ display: 'flex', gap: '24px', minHeight: '750px' }}>
              {/* 左侧：时间管理建议 */}
              <div style={{ flex: 0.7, height: '100%' }}>
              ...
              {/* 右侧：中奖号码卡片展示 */}
              <div style={{ flex: 4, display: 'flex', flexDirection: 'column', gap: '20px', position: 'relative', height: '100%' }}>
                {/* 中奖号码展示区域 */}
                <div style={{ flex: 1, padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', scrollbarWidth: 'none', msOverflowStyle: 'none', maxHeight: 'calc(100vh - 200px)' }}>
                  {selectedCard ? (
                    <>
                      {/* 红球和蓝球号码卡片（同一行） */}
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      ...
                      <div style={{
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '8px' }}>
                          {Array.from({ length: 49 }, (_, index) => {
                            ...
                            let count = 0;
                            let baseColor = '#ff4d4f';
                            let isRedBall = true;
                            
                            if (ballNumber <= 33) {
                              count = redBallCounts.get(ballNumber) || 0;
                              baseColor = '#ff4d4f';
                              isRedBall = true;
                            } else {
                              count = blueBallCounts.get(ballNumber - 33) || 0;
                              baseColor = '#1890ff';
                              isRedBall = false;
                            }
                            
                            const maxCount = Math.max(
                              ...Array.from(redBallCounts.values()),
                              ...Array.from(blueBallCounts.values()),
                              1
                            );
                            const intensity = count / maxCount;
                            const lightness = isRedBall 
                              ? Math.max(20, 60 - intensity * 40)
                              : Math.max(20, 60 - intensity * 40);
                            const backgroundColor = isRedBall 
                              ? `hsl(0, 100%, ${lightness}%)`
                              : `hsl(207, 90%, ${lightness}%)`;
                            
                            const isWinningNumber = isRedBall 
                              ? getSelectedPeriodNumbers.redNumbers.includes(ballNumber)
                              : getSelectedPeriodNumbers.blueNumber === (ballNumber - 33);
                            
                            const borderLeftWidth = isWinningNumber ? '3px' : '1px';
                            const borderLeftColor = isWinningNumber ? '#FFC107' : `${baseColor}40`;
                            ...
                            return (
                              <div
                                ...
                                borderLeftColor: borderLeftColor,
                                  backgroundColor: '#1A1A1A',
                                  backgroundImage: `linear-gradient(145deg, rgba(30, 30, 30, 0.9), rgba(10, 10, 10, 0.9)), radial-gradient(circle at 25% 25%, rgba(255, 255, 255, 0.05), transparent 50%), radial-gradient(circle at 100% 100%, ${backgroundColor}, transparent 70%)`,
                                  textAlign: 'center',
                                  transition: 'transform 0.3s ease, box-shadow 0.3s ease, backgroundPosition 0.3s ease',
                                  cursor: 'pointer',
                                  transformStyle: 'preserve-3d',
                                  perspective: '1000px',
                                  boxShadow: `0 4px 12px rgba(0, 0, 0, 0.4), 0 2px 8px rgba(0, 0, 0, 0.3), 0 0 10px ${baseColor}30, inset 0 1px 0 rgba(255, 255, 255, 0.1), inset 0 -1px 0 rgba(0, 0, 0, 0.3)`,
                                  position: 'relative',
                                  overflow: 'hidden'
                                }}
                                onMouseEnter={(e) => {
                                  const card = e.currentTarget;
                                  card.style.transform = 'translateZ(8px) scale(1.05) translateY(-2px)';
                                  card.style.boxShadow = `0 8px 24px rgba(0, 0, 0, 0.6), 0 4px 16px rgba(0, 0, 0, 0.4), 0 0 20px ${baseColor}60, inset 0 1px 0 rgba(255, 255, 255, 0.15), inset 0 -1px 0 rgba(0, 0, 0, 0.4)`;
                                }}
                                onMouseLeave={(e) => {
                                  const card = e.currentTarget;
                                  card.style.transform = 'translateZ(0) scale(1) translateY(0)';
                                  card.style.boxShadow = `0 4px 12px rgba(0, 0, 0, 0.4), 0 2px 8px rgba(0, 0, 0, 0.3), 0 0 10px ${baseColor}30, inset 0 1px 0 rgba(255, 255, 255, 0.1), inset 0 -1px 0 rgba(0, 0, 0, 0.3)`;
                                }}
                              >
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                                  <div style={{
                                    ...
                                    fontSize: '14px',
                                    fontWeight: 'bold',
                                    boxShadow: `0 0 8px ${baseColor}60, inset 0 1px 2px rgba(255, 255, 255, 0.2), inset 0 -1px 2px rgba(0, 0, 0, 0.3)`,
                                    border: `1px solid ${baseColor}40`
                                  }}>
                                    {isRedBall ? ballNumber : ballNumber - 33}
                                  </div>
                                  <div style={{
                                    fontSize: '12px', 
                                    color: '#999'
