...
const getStatisticsTabState = (search: string) => {
  const tab = new URLSearchParams(search).get('tab');
  const tabMap: Record<string, { statisticType: 'red' | 'blue'; activeTabKey: string }> = {
    frequency: { statisticType: 'red', activeTabKey: '1' },
    group: { statisticType: 'red', activeTabKey: '5' },
    distribution: { statisticType: 'red', activeTabKey: '3' },
    'red-frequency': { statisticType: 'red', activeTabKey: '1' },
    'blue-frequency': { statisticType: 'blue', activeTabKey: '2' },
    'red-distribution': { statisticType: 'red', activeTabKey: '3' },
    'blue-distribution': { statisticType: 'blue', activeTabKey: '4' },
    'red-group': { statisticType: 'red', activeTabKey: '5' },
    'blue-group': { statisticType: 'blue', activeTabKey: '6' }
  };
...
const Statistics: React.FC<{ isTabVisible: boolean }> = ({ isTabVisible }) => {
  ...
  useEffect(() => {
    const nextTabState = getStatisticsTabState(location.search);
    if (!nextTabState) return;

    setStatisticType(nextTabState.statisticType);
    setActiveTabKey(nextTabState.activeTabKey);
  }, [location.search]);
      ...
      <footer className="app-footer" style={{
        ...
        <div style={{
          ...
          <div
            ...
            onClick={() => setActiveTabKey(statisticType === 'red' ? activeStatisticMenuItem.redTabKey : activeStatisticMenuItem.blueTabKey)}
          >
            ...
            >
              {statisticMenuItems.map(item => {
                ...
                const tabKey = statisticType === 'red' ? item.redTabKey : item.blueTabKey;
