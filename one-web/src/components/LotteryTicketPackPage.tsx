...
const LotteryTicketPackPage = () => {
  ...
  const createPack = async () => {
    ...
    setError(undefined);
    try {
      await lotteryTicketPackApi.create(preview || draftPayload());
      setPreview(undefined);
      await loadPacks();
      message.success('票包草稿已创建');
    } catch (requestError) {
      ...
      <section className="lottery-ticket-pack-grid">
        <Card className="life-panel-card lottery-clean-panel" title={<Space><ImportOutlined />生成草稿</Space>}>
          <div className="lottery-ticket-pack-form">
            ...
            <label><span>目标期号</span><Input value={targetIssue} onChange={event => setTargetIssue(event.target.value)} placeholder="例如 2026068" /></label>
            <label>
              <span>来源</span>
              <Select
                value={sourceType}
          ...
          <div className="lottery-ticket-pack-list">
            {packs.length ? packs.map(pack => (
              <article key={pack.id}>
                ...
                <Space wrap>
                  <Button size="small" icon={<CheckCircleOutlined />} loading={actionLoading === `approve-${pack.id}`} disabled={pack.approvalState === 'APPROVED' || pack.status === 'SAVED'} onClick={() => runPackAction(pack, 'approve')}>审批</Button>
                  <Button size="small" type="primary" icon={<SaveOutlined />} loading={actionLoading === `save-${pack.id}`} disabled={pack.approvalState !== 'APPROVED' || pack.status === 'SAVED'} onClick={() => runPackAction(pack, 'save')}>保存票据</Button>
                  <Button size="small" onClick={() => navigate(`/lottery/tickets?issue=${pack.targetIssue || ''}&source=TICKET_PACK`)}>票据</Button>
                  <Button size="small" danger loading={actionLoading === `archive-${pack.id}`} disabled={pack.status === 'ARCHIVED'} onClick={() => runPackAction(pack, 'archive')}>归档</Button>
                </Space>
              </article>
