package org.aurorae.flink.record;
...
public class RecordProcessor {
    ...
    public static void main(String[] args) throws Exception {
        ...
        DataStreamSource<String> source = env.fromSource(KafkaUtil.addSource("record"), WatermarkStrategy.noWatermarks(), "record-source");
        source.print();
        SingleOutputStreamOperator<Integer> countStream = source.map(value -> Arrays.stream(value.split(",")).map(Integer::parseInt).collect(Collectors.toList()))
                ...
                .process(new RecordCountFunction())
                .returns(Integer.class);
        countStream.print("sum ");
        // countStream.sinkTo(KafkaUtil.addSink("record-count"));
